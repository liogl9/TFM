...
TRAIN_NAME = "Regressor_0"
FOLDER = "Regressor_Sintec_0_30_random"
BST_MODEL = 199
...
class NormalsDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        image_path = os.path.join(self.path, "images", self.labels[index][0])
...
def my_loss(output,target):
    loss = torch.mean(torch.sqrt(torch.sum((output-target)**2,1))) 
    return loss
##########################
#       TRAINNING        #
##########################
...
def progress(epoch, num_epochs, train_loss, val_loss, it_time, percent=0, width=40):
    ...
    print("It/s: {:.4f}".format(1 / (it_time + 1e-5)), flush=True)
...
if __name__ == "__main__":
    ...
    num_imgs = len(os.listdir(os.path.join(os.getcwd(),'datasets',FOLDER,'images')))
    # num_imgs = 306

    # dataset_size_trains = [x * num_imgs // 10 for x in range(1, 11)]
    dataset_size_trains = [num_imgs]
    ...
    for loop_i, dataset_size in enumerate(dataset_size_trains):
        ...
        if FINE_TUNING:
            txt_path = (
                os.path.join(BASE_DIR,"datasets","FOLDER", VERSION, "labels", PP)
                + ".txt"
            )
        else:
            txt_path = (
                # os.path.join(BASE_DIR ,"datasets", "Regressor_Sintec_0_30_random", "labels", "G1_a_Point_1")
                os.path.join(BASE_DIR ,"datasets", FOLDER, "labels", PP)
                + ".txt"
            )
        ...
        # }
        test_dict = {
            "Img_name": [item[0] for item in val],
            "x": [item[1] for item in val],
            "y": [item[2] for item in val],
            "u": [item[3] for item in val],
            "v": [item[4] for item in val],
            "w": [item[5] for item in val],
        }

        df = pd.DataFrame(test_dict)
        df.to_csv(dir + "/test_imgs.txt", sep=" ", index=False)

        del labels, full_labels, rand_list, test_dict, df
        # del labels, train_val, full_labels, rand_list, test_dict, df

        # Creation of dataset and dataloaders
        if FINE_TUNING:
            path = os.path.join(BASE_DIR,"datasets",FOLDER,VERSION)

        else:
            path = os.path.join(BASE_DIR,"datasets",FOLDER)
        ...
        datasets = {
            x: NormalsDataset(
                ...
                type=x,
                point=PP,
                data=train if x == "train" else val if x == "val" else None,
            )
            for x in ["train", "val"]
        ...
        if FINE_TUNING:
            ...
            model_path = os.path.join(os.getcwd(),"models", TRAIN_NAME, PP, FOLDER, model_name)
            ...
            model = CNNRegression224Conv(output_size=output_size).to(device)
        ...
        for epoch in range(num_epochs):
            ...
            for phase in ["train", "val"]:
                ...
                if phase == "val":
                    if best_loss - epoch_loss > 0.0001:
                        ...
                        best_model = model
                        best_epoch = epoch
                        PATH = os.path.join(dir,"{}_{:02d}.pt".format(PP,epoch))
                        try:
            ...
            print("Time val: {:.0f}m {:.0f}s".format(time_test // 60, time_test % 60))
        ...
        with torch.no_grad():
            ...
            print("Number of predicts: ", str(dataset_sizes["val"]))
            print("Val time: {:.0f}m {:.0f}s".format(time_test // 60, time_test % 60))
