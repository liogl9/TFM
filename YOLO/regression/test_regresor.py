...
import shutil
from regression_train import my_loss
import matplotlib.pyplot as plt
import pandas as pd
...
def save_suspicius_image(*,model_name, good_img: bool = False, delete=False, index=None, img=None,
    ...
    # Parent folder
    if good_img:
        parent_folder = os.path.join(os.getcwd(), "models", TRAIN_NAME, PP,FOLDER,model_name, "good_imgs")
    else:
        parent_folder = os.path.join(os.getcwd(), "models", TRAIN_NAME, PP,FOLDER,model_name, "suspicious_imgs")
    if delete:
...
def dset_batches():
    ...
    path = os.path.join("./models/" + TRAIN_NAME + "/G1_a_Point_2")
...
def save_figure(model_name, boxplot, name, xticks_data):
    ...
    plt.savefig(os.path.join(os.getcwd(), "models", TRAIN_NAME, PP, FOLDER, model_name,
                "".join([name, '.png'])))
...
def save_scatter_plot(*,model_name, y_value: list, x_value: list, name: str):
    plt.clf()
    ...
    plt.savefig(os.path.join(os.getcwd(), "models", TRAIN_NAME, PP, FOLDER,model_name,
                "".join(["median_scatterplot_", name, '.png'])))
    plt.close()


def save_violin(*,model_name, classes, values):
    plt.clf()
    ...
    plt.savefig(os.path.join(os.getcwd(), "models", TRAIN_NAME, PP, FOLDER,model_name,
                "".join(['violin_plot', '.png'])))


def add_og_and_map(*,model_name, og, heat_map, delete: bool = False, name):
    ...
    parent_folder = os.path.join(os.getcwd(), "models", TRAIN_NAME, PP,FOLDER,model_name, "saliency_imgs")
    if delete:
        if os.path.exists(parent_folder):
            shutil.rmtree(parent_folder)
    if not os.path.isdir(parent_folder):
...
if __name__ == "__main__":
    ...
    # imgs_path = os.path.join(os.getcwd(), 'datasets', 'Regressor_test', 'images')
    imgs_path = os.path.join(os.getcwd(), 'datasets', 'Regressor_Gan', 'images')
    ...
    # txt_path = os.path.join(os.getcwd(),'datasets','Regressor_test','labels','Labels_regresor.txt')
    txt_path = os.path.join(os.getcwd(),'datasets','Regressor_Gan','labels','G1_a_Point_2.txt')
    ...
    datasets = {'test': NormalsDataset(path= os.path.join(os.getcwd(), 'datasets', 'Regressor_Gan'),
                                    type='test',
                                    point=PP,
                                    data=test)}
    ...
    file_list = os.listdir(os.path.join("./models", TRAIN_NAME, PP, FOLDER))
    model_list = [i for i in file_list if '.pt' in i]
    ...
    for num, model_name in enumerate(model_list):
        ...
        regresor.eval()
        save_suspicius_image(model_name=model_name, delete=True)
        ...
        for i, (original, img, lbl, extra) in enumerate(dataloaders['test']):
            ...
            if SALIENCY:
                ...
                og_and_map = add_og_and_map(model_name=model_name, og=image_host, heat_map=heatmap, delete=sal_folder_flag, name=labels_txt[i][0])
            ...
            if SHOW_VECTORS:
                ...
                # cv2.imshow('{}'.format(labels_txt[i][0]), img)
                # cv2.waitKey(0)
                ...
                # plt.show()
                if SAVE_VECTORS:
                    vec_folder = os.path.join(os.getcwd(), "models", TRAIN_NAME, PP,FOLDER,model_name,'vectors')
                    if not os.path.isdir(vec_folder):
                        os.mkdir(vec_folder)
                    plt.savefig(os.path.join(vec_folder,labels_txt[i][0]))
                plt.close()
                cv2.destroyAllWindows()
            ...
            if TEST_PLOTS:
                ...
                if angle > ANGLE_LIMIT:
                    img_path = os.path.join(imgs_path, labels_txt[i][0])
                    img = cv2.imread(img_path)
                    save_suspicius_image(model_name=model_name,index=index, img=img, img_name=labels_txt[i][0],
                                        label=labelled, prediction=predicted)
                else:
                    img_path = os.path.join(imgs_path, labels_txt[i][0])
                    img = cv2.imread(img_path)
                    save_suspicius_image(model_name=model_name,good_img=True, index=index, img=img, img_name=labels_txt[i][0],
                                        label=labelled, prediction=predicted)
                # Save it
            ...
            counter += 1
        if TEST_PLOTS:
            vect_scatter_list, _ = save_figure(model_name=model_name,boxplot=boxplot_vec, name='diff_vector', xticks_data=bins_list)
            angl_scatter_list, bins_counter = save_figure(model_name=model_name, boxplot=boxplot_ang, name='angles', xticks_data=bins_list)
            save_scatter_plot(model_name=model_name,y_value=vect_scatter_list, x_value=bins_counter, name='diff_vector')
            save_scatter_plot(model_name=model_name,y_value=angl_scatter_list, x_value=bins_counter, name='angles')

            save_violin(model_name=model_name,classes=bins_list, values=angles)
        ...
        model_num.append(model_name[-2:])
        losses.append(running_loss/len(test))
        
        ##############
        # LOG & PLOT #
        ##############
        # Plot train and val error curves
        epochs = range(0, num + 1)
        plt.close()
        plt.style.use("ggplot")
        plt.plot(epochs, losses, "salmon", label="Model loss")
        plt.xticks(epochs, model_num)  # Set text labels.
        plt.title("Test losses")
        plt.xlabel("Epochs")
        plt.ylabel("Loss")
        plt.legend()
        # plt.show()
        plt.savefig(os.path.join(os.getcwd(), "models", TRAIN_NAME, PP,FOLDER,'models_test_loss.png'))
        plt.close()
    # cv2.destroyAllWindows()
